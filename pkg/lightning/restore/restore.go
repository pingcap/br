	...
	if indexEngineCp.Status < checkpoints.CheckpointStatusClosed {
		...
		for engineID, engine := range cp.Engines {
			...
			if engine.Status < checkpoints.CheckpointStatusAllWritten {
				indexEngine, err = rc.backend.OpenEngine(ctx, idxEngineCfg, tr.tableName, indexEngineID, rc.ts)
				if err != nil {
					return errors.Trace(err)
				}
				break
...
func (tr *TableRestore) restoreEngine(
	...
	dataEngine, err := rc.backend.OpenEngine(ctx, dataEngineCfg, tr.tableName, engineID, rc.ts)
	if err != nil {
		return nil, errors.Trace(err)
	}
