...
export GO_FAILPOINTS='github.com/pingcap/br/pkg/lightning/lightning/backend/FailIngestMeta=1*return("notleader")'
...
export GO_FAILPOINTS='github.com/pingcap/br/pkg/lightning/lightning/backend/FailIngestMeta=2*return("epochnotmatch")'
...
set +e
export GO_FAILPOINTS='github.com/pingcap/br/pkg/lightning/lightning/restore/FailBeforeDataEngineImported=return'
...
for ckpt in mysql file; do
  ...
  set +e
  export GO_FAILPOINTS="github.com/pingcap/br/pkg/lightning/lightning/restore/FailAfterWriteRows=return"
  ...
  set +e
  export GO_FAILPOINTS="github.com/pingcap/br/pkg/lightning/lightning/restore/LocalBackendSaveCheckpoint=return;github.com/pingcap/br/pkg/lightning/lightning/restore/FailIfImportedChunk=return(1)"
  run_lightning --backend local --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "tests/$TEST_NAME/$ckpt.toml"
  set -e
  run_lightning_ctl --check-local-storage \
    --backend local \
  ...
  set +e
  export GO_FAILPOINTS="github.com/pingcap/br/pkg/lightning/lightning/restore/FailIfIndexEngineImported=return(1)"
  run_lightning --backend local --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "tests/$TEST_NAME/$ckpt.toml"
  set -e
  run_lightning_ctl --check-local-storage \
    --backend local \
