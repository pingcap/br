...
func (r *testStorageSuite) TestCreateStorage(c *C) {
	...
	c.Assert(err, ErrorMatches, "\\[[a-zA-Z:]+\\]storage net not support yet")
	...
	c.Assert(err, ErrorMatches, `\[[a-zA-Z:]+\]please specify the bucket for s3 in s3:///bucket/more/prefix/`)
