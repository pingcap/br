...
# backup table
echo "backup start..."
br --pd $PD_ADDR backup table -s "local://$TEST_DIR/$DB" --db $DB -t $TABLE --ratelimit 5 --concurrency 4 --fastchecksum true

run_sql "DROP TABLE $DB.$TABLE;"

# restore table
echo "restore start..."
br restore table --db $DB --table $TABLE -s "local://$TEST_DIR/$DB" --pd $PD_ADDR
