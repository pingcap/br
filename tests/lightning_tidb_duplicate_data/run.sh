...
for type in replace ignore error; do
    ...
    export GO_FAILPOINTS="github.com/pingcap/br/pkg/lightning/backend/tidb/FailIfImportedSomeRows=return"
    set +e
    echo yes | run_lightning --config "tests/$TEST_NAME/$type.toml" 2> /dev/null
    ERRORCODE=$?
    set -e
    [ "$ERRORCODE" -ne 0 ]
    ...
    if [ $type = 'error' ]; then
        set +e
        echo yes | run_lightning --config "tests/$TEST_NAME/$type.toml" --log-file "$TEST_DIR/lightning-error-on-dup.log"
        ERRORCODE=$?
        set -e
        [ "$ERRORCODE" -ne 0 ]
        ...
        echo yes | run_lightning --config "tests/$TEST_NAME/$type.toml"
