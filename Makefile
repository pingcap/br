...
# TODO: unify LDFLAGS
LDFLAGS += -X "$(BR_PKG)/pkg/lightning/lightning/common.ReleaseVersion=$(VERSION)"
LDFLAGS += -X "$(BR_PKG)/pkg/lightning/lightning/common.BuildTS=$(shell date -u '+%Y-%m-%d %I:%M:%S')"
LDFLAGS += -X "$(BR_PKG)/pkg/lightning/lightning/common.GitHash=$(shell git rev-parse HEAD)"
LDFLAGS += -X "$(BR_PKG)/pkg/lightning/lightning/common.GitBranch=$(shell git rev-parse --abbrev-ref HEAD)"
LDFLAGS += -X "$(BR_PKG)/pkg/lightning/lightning/common.GoVersion=$(shell go version)"
...
data_parsers: tools pkg/lightning/lightning/mydump/parser_generated.go
	PATH="$(GOPATH)/bin":"$(PATH)":"$(TOOLS)" protoc -I. -I"$(GOPATH)/src" pkg/lightning/lightning/checkpoints/file_checkpoints.proto --gogofaster_out=.
	$(TOOLS)/vfsgendev -source='"github.com/pingcap/br/pkg/lightning/lightning/web".Res' && mv res_vfsdata.go pkg/lightning/lightning/web/

web:
	cd pkg/lightning/web && npm install && npm run build
